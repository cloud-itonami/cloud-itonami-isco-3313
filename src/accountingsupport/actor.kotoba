(ns accountingsupport.actor
  "AccountingSupportActor — the ISCO-08 3313 community accountingsupport actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one accountingsupport operation request
  ...
  The unconditional invariant: the AccountingSupportAdvisor can never
  directly commit a record the AccountingSupportGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [accountingsupport.advisor :as advisor]
            [accountingsupport.governor :as governor]
            [accountingsupport.store :as store]))

(defn build-graph
  "Build a compiled AccountingSupportActor graph. `store` implements
  `accountingsupport.store/Store`. `advisor` implements
  `accountingsupport.advisor/Advisor` (defaults to `mock-advisor`).
